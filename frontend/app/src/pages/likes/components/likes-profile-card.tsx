...
export function LikesProfileCard({
	...
	return (
		<div
			className="relative col-span-1 h-[33vh] m-1 rounded-sm overflow-hidden cursor-pointer"
		...
		>
				...
				<button
					...
					onClick={(e) => {
						...
						unlikeUser(profileId)
							...
							.catch((err: unknown) =>
								toast.error(err instanceof Error ? err.message : "Failed to unlike"),
							);
					}}
				>
					<img src={unlikeSvg} alt="Unlike profile" className="w-5 h-5 drop-shadow-lg" />
				</button>
			)}
			<button
				type="button"
			...
			>
				<img src={blockSvg} alt="Block profile" className="w-5 h-5 drop-shadow-lg" />
			</button>
			<button
				type="button"
			...
			>
				<img src={reportSvg} alt="Report fake account" className="w-5 h-5 drop-shadow-lg" />
			</button>
			<div className="relative flex items-end h-full p-2">
				<span className="text-white font-semibold drop-shadow-lg">{firstname}</span>
			</div>
		</div>
	);
}
