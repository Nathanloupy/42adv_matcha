...
@router.get("/users/me", response_model=dependencies.User, response_model_exclude={"password"}, tags=["users"])
async def me(session: dependencies.session, user: dependencies.user):
	return user

@router.patch("/users/me", tags=["users"])
async def me_patch(session: dependencies.session, user: dependencies.user, update_profile: UpdateProfile):
...
async def me_delete_tag(session: dependencies.session, user: dependencies.user, tag: str):
...
async def me_add_image(session: dependencies.session, user: dependencies.user, image: UploadFile):
...
async def me_delete_image(session: dependencies.session, user: dependencies.user, image_uuid: str):
